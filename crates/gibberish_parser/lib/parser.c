...
static const char *group_names[] = {
    "named",          "_atom",       "call_name",     "args",
    "call",           "member_call", "seq",           "choice",
    "_expr",          "kw_def",      "token_def",     "fold_stmt",
    "parser_def",     "child_query", "group_query",   "label",
    "labelled_query", "_query",      "highlight_def", "_stmt",
    "root",           "unmatched",   "error",
};
...
static const char *label_names[] = {
    "expression", "token_name", "regex", "parser_name", "declaration", "error",
};
...
static const char *token_names[] = {
    "KEYWORD",   "PARSER",  "TOKEN",   "HIGHTLIGHT", "FOLD",    "whitespace",
    "int",       "colon",   "comma",   "bar",        "dot",     "l_bracket",
    "r_bracket", "l_paren", "r_paren", "l_brace",    "r_brace", "plus",
    "eq",        "ident",   "semi",    "string",     "at",      "error",
};
...
/* RegexRegexOption */
static bool lex_96(LexerState *lexer_state) { return lex_97(lexer_state); }
...
ParserState default_state(char *ptr, size_t len) {
  ...
  return (ParserState){
      ...
      .stack = stack,
      .offset = 0,
      .breaks = break_stack_new(), /* <-- make sure this matches your actual
                                      constructor name */
...
static size_t parse_7(ParserState *state, size_t unmatched_checkpoint) {
  ...
  /* If res is EOF (2) or a break for a later part (>=2 but not this part), this
   * part is missing. */
  if (res >= 2 && res != brk_1) {
...
/* Parse Sep */
static size_t parse_54(ParserState *state, size_t unmatched_checkpoint) {
  /* Push break predicates: item then sep (sep ends up on top, like your old
   * push order). */
  ...
  for (;;) {
    ...
    /* item didn't parse */
    {
      /* Always emit missing(item) on failure (matches old QBE check_item_eof
       * path) */
      ExpectedVec e = expected_25();
      missing(state, e);
      ...
      if (res == sep_brk) {
        /* We hit a sep delimiter => treat missing item as recovery and continue
         * with sep */
        continue;
      }
...
/* Parse Fold */
static size_t parse_33(ParserState *state, size_t unmatched_checkpoint) {
  /* Skip leading skipped tokens until either EOF or peak(first) says we can
   * start. */
  for (;;) {
    if (state->offset >= state->tokens.len) {
      return 2; /* EOF */
  ...
  /* Push break predicate for "next" and get the break code that child parsers
   * will return */
...
Node parse(char *ptr, size_t len) {
  ...
  /* After parse: consume skipped tokens until EOF; otherwise emit error until
   * EOF */
  for (;;) {
    if (state.offset >= state.tokens.len) {
      break;
